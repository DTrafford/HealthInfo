...
//   });
  ...
  delete tip._id;
// var updatedTip = {};
// updatedTip = Object.assign(updatedTip, req.body);
// updatedTip.imagePath = imagePath;
// delete updatedTip._id;
Tip.updateOne({ _id: req.params.id, creatorId: req.userData.userId },
              {title: tip.title,
              content: tip.content,
              creatorId: req.userData.userId,
              creatorName: req.userData.displayName,
...
exports.deleteTip = (req, res, next) => {
  Tip.deleteOne({_id: req.params.id}).then(result => {
    if(result.n > 0) {
